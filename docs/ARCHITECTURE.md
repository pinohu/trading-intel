...
- Historical backtesting, companion market-app, portfolio analytics, research data, LLM agent research, and ML forecast workers, including optional OpenStock, Ghostfolio, AKShare, LLM Trading Lab, StockSharp C#/.NET, StockPredictionAI-style, and Stock Prediction Models worker integrations.
...
flowchart LR
  ...
  TradingAgents --> DB
  BacktestWorker["Companion + Portfolio + Research Data + Agent + Backtest + Forecast Workers: OpenStock/Ghostfolio/AKShare/LLM Trading Lab/StockPredictionAI/Stock Prediction Models/LEAN/StockSharp/vectorbt/backtesting.py"] --> DB
...
- Never let TradingAgents, LLM Trading Lab, OpenStock, Ghostfolio, AKShare, StockPredictionAI, Stock Prediction Models, or StockSharp worker output place autonomous broker orders; manual paper/live execution stays in the broker controls and audit rail.
