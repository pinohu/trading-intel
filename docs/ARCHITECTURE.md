...
- Historical backtesting workers, including optional StockSharp C#/.NET worker integration.
...
flowchart LR
  ...
  TradingAgents --> DB
  BacktestWorker["Backtest Workers: LEAN/StockSharp/vectorbt/backtesting.py"] --> DB
...
- Never let TradingAgents or StockSharp worker output place autonomous broker orders; manual paper/live execution stays in the broker controls and audit rail.
